...
def exampple_post_call():
    import requests
    try:
      response = requests.post(API_URL, data=json.dumps(body), headers=headers, timeout=TIMEOUT)
      # requests.get('http://localhost:5001/', data='{}', headers={}, timeout=10)
      response = response.json()
      search_response = response.get('data', {}).get('hotelX', {}).get('search', {})
...
def example_response_mock():
    import responses
    import requests

    @responses.activate
    def hi(url='http://hi'):
        responses.add(responses.POST, url, 'hi')
        response = requests.post(url, data='', headers={}, timeout=10)
        print(response.content)
    hi()

import importlib
def example_import_by_string():
    keras = importlib.import_module('keras')

    # FAIL: for some reason
    importlib.import_module('keras.thing1.thing2')

    # import with from
    

    # import function by string
...
def class_factory(name, base_kwargs, derived_arg_names, base_class):
    '''
    Factory for generating classes derived from base_class.
    Example:
    DerivedClass = class_factory('DerivedClass', {'a': 2}, ['derived_class_var'], BaseClass)
    instance = DerivedClass(derived_class_var=4)
    :param name: name of the new class
    :param base_kwargs: dict mapping strings (ie argument names of the __init__ of base_class) to the values to always
                      pass to the __init__ of base_class
    :param arg_names: list[str] representing the variables passed to
    :param base_class: class name of the base class
    ...
    for key, value in kwargs.items():
        ...
        # ClassFactory call
        if key not in derived_arg_names:
...
def generate_unittest_tests_in_a_class_dynamically():
    import unittest
 ...
 def generate_unittest_classes_dynamically():
    ...
    def main():
        class BaseClass(snapshottest.TestCase):
            def test_stuff(self):
                self.assertTrue(self.high)

        for i in range(5):
            name = f'TestDerivedClass{i}'
            hi = TestCaseClassFactory.generate_class(name, BaseClass)
            hi.high = False
            globals()[name] = hi
...
import urllib
def url_safe_encode(x):
    # Encode x so that it can be safely passed through URL without being modified
    # quote_plus is used in urllib.parse.urlencode
    return urllib.parse.quote_plus(x)

def url_safe_unscramble(encoded_x):
    # unquote_plus is used in urllib.parse.parse_qs
  return urllib.parse.unquote_plus(encoded_x)

import ast
import astor # pip install astor

def code_to_ast(code_str):
    # abstract syntax trees filter out a lot of extra text such as whitespace and comments
    ast.parse(code_str)
...
def logging_examples():
    ...
    logger.setLevel(logging.DEBUG) # What does this do??
    '''
    Both handlers and loggers have log levels; lines only reach the output if they're above both thresholds.
    Since you don't set the root log level, it uses its default (warning).
    https://stackoverflow.com/questions/30677573/python-logging-setlevel-not-logging
    '''
    ...
    # without specifying handler levels, info and debug don't print by default (error, warning, and critical do though)
    logger.info('Hi')  # prints '2019-06-17 18:08:08,719 - spam_application - INFO - Hi' to console stdout
    logger.error('Hi')  # prints '2019-06-17 18:08:08,719 - spam_application - ERROR - Hi' to console stderr
    logger.debug('Does nothing')
    logger.warning('Hi')
    logger.critical('Hi')

def create_info_logger(name):
...
def pretty_print(x):
    import pprint; pprint.PrettyPrinter(indent=4).pprint(x)

def pretty_print_string(x):
    import pprint
...
def debugging_line():
    import sys; import traceback; exc_info = sys.exc_info(); traceback.print_exception(*exc_info); import pdb; pdb.set_trace()

def debugging_line2():
    import sys; import traceback; exc_info = sys.exc_info(); traceback.print_exception(*exc_info); import pprint; pprint.PrettyPrinter(indent=3).pprint(locals()); import pdb; pdb.set_trace()

def debugging_line3():
    import sys; import traceback; exc_info = sys.exc_info(); _file_path = '/Users/hromel/log.txt'; f = open(_file_path, 'a'); traceback.print_exception(*exc_info, file=f); f.close(); import pprint; pprint.PrettyPrinter(indent=3).pprint(locals()); import pdb; pdb.set_trace()

def log_to_file_line():
    _to_log = f"XXXXXX"; _file_path = '/Users/hromel/log.txt'; f = open(_file_path, 'a'); f.write(str(_to_log) + '\n'); f.close()

def log_to_file_line2():
    _to_log = f"{}"; _file_path = '/Users/hromel/log.txt'; import pprint; f = open(_file_path, 'a'); f.write(pprint.pformat(_to_log, indent=2) + '\n'); f.close()

def is_python_class(cls):
    # https://stackoverflow.com/questions/395735/how-to-check-whether-a-variable-is-a-class-or-not
    return inspect.isclass(cls)

def inherited_classes(cls):
    # (includes cls itself)
    return inspect.getmro(cls)

def is_python_class_which_inherits(cls, possibly_inherited_cls):
    return (is_python_class(cls) and is_python_class(possibly_inherited_cls) and
            possibly_inherited_cls in inherited_classes(cls))

def string_to_unicode(s):
    # Example: 'hai✓'.encode('utf-8').decode('utf-8')
    return s.encode('utf-8')

def unicode_to_string(u):
    return u.decode('utf-8')

def parse_protobuf_without_proto(text):
    # ACTUALTODO: Look into https://stackoverflow.com/questions/13937882/parsing-protocol-buffers-without-knowing-the-proto
    pass

def lazy_get(obj, key, where=None):
    '''
    Check all values are at the same tree level
    Check there is at least 1
    '''

def get_source_code_example():
    import inspect
    def foo(a, b):
        return 1
    foo_source_code_text = inspect.getsource(foo)
...
# pythonic
def argmin(l):
    return min([(i, val) for i, val in enumerate(l)], key=lambda x: x[1])[0]

def argmin_v2(l):
    indices = list(range(len(l)))  # maybe you already have this
    return max(indices, key=lambda ix: l[ix])
...
def cmd_output(cmd):
    ...
    if sys.version_info >= (3, 5):
        ...
        result = subprocess.check_output(list(cmd.split()))
        return result
    else:
        raise NotImplementedError()  # in stackoverflow answer

def filter_first(l, fn):
    # this creates a generator, which is more efficient than a list for this
...
def multiprocessing_example():
    ...
    pool = ThreadPool(3)  # you can run 5 threads and they will all get executed
    threads = [pool.apply_async(work, args=("stuff",))]
    results = [thread.get() for thread in threads]
...
def my_replace(text):
    for key, value in config.items():
        if key not in text:
            import warnings; warnings.warn(f'{key} not in text')
        text = text.replace(key, str(value))
        
    assert 'HENRI-' not in text, text
    return text

def dict_subset_of(d1, d2):
    # d1 subset of d2?
    for key in d1:
...
def lazy_filter(obj, match_d):
    if isinstance(obj, (list, tuple)):
        for val in obj:
            result = lazy_filter(val, match_d)
            if result:
                return result
    if not isinstance(obj, dict):
        return None
    if dict_subset_of(match_d, obj):
        return obj
    for val in obj.values():
        result = lazy_filter(val, match_d)
        if result:
...
def pp(*args):
    import pprint; pprint.PrettyPrinter(indent=4).pprint(*args)
...
def dict_recursive_subset_of(d1, d2):
    ...
    try:
        ...
        if on_this_level:
            return True
        for val_d in d2.values():
            if dict_recursive_subset_of(d1, val_d):
                return True
        return False
    except Exception as ex:
        import sys; import traceback; exc_info = sys.exc_info(); traceback.print_exception(*exc_info)
...
def lazy_filter_v2(obj, match):
    if isinstance(obj, (list, tuple)):
        for val in obj:
            result = lazy_filter(val, match)
            if result:
                return result
    if not isinstance(obj, dict):
        return None
    if isinstance(match, dict):
        match = [match]
    if all([dict_recursive_subset_of(match_d, obj) for match_d in match]):
        return obj
    for val in obj.values():
        result = lazy_filter_v2(val, match)
        if result:
            return result
    
    return None

def test_lazy_filter_v2():
    lazy_filter_v2({'hi': {'yo': 1}, 'there': 0}, [{'yo': 1}, {'there': 0}])
    lazy_filter_v2({'hi': {'yo': 1}, 'there': 0}, [{'yo': 1}, {'there': 1}])
    lazy_filter_v2({'hi': {'yo': 1}, 'there': 0}, [{'yo': 0}, {'there': 0}])

def pseudo_example_postman_pipeline():
    '''
    TODO: make code cleaner
    problem: needing to rename search_payload when pasting from Postman
    '''
    search_payload = 'blah'
    ...
    # ....
    obj = lazy_filter(json.loads(response.text), {'id': my_id})
    price = lazy_get(obj, 'net')
    config['HENRI-ID'] = '123'
    payload = my_replace(search_payload)
    add_to_clipboard(payload)

def protobuf_parse(empty_proto, text):
    # INCOMPLETE
    empty_proto.ParseFromString()

def protobuf_to_string(proto):
    # INCOMPLETE
    proto.SerializeToString()

def lazy_protobuf_parse(text):
    # Requires exploring repos and loading types to check
    raise NotImplementedError()

def for_log_printing():
    '''
    if a response, print its content
    if a dictionary, pretty print
    '''
    raise NotImplementedError()

def lazy_try_parse(text):
